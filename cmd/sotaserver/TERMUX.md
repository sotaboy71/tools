# Running sotaserver on Android with Termux

You can build and run `sotaserver` directly on your phone — no PC required.
...
./sotaserver I need help          # confirms it works + shows the walkthrough
```
...
Clone your repository and switch to the branch that has sotaserver on it:
...
`cmd/sotaserver`. (If the code later gets merged into the main branch, you can
...
You now have a single executable named `sotaserver` in the current folder.
...
```sh
./sotaserver I need help          # step-by-step walkthrough
./sotaserver dns -domain example.com
./sotaserver portscan -host example.com -ports 1-1024
./sotaserver tlsinfo -host example.com -port 443
./sotaserver attacks              # the 12 attack types: detect & defend
./sotaserver toolbox              # directory of the big-name tools
```
...
```sh
cd tools
./sotaserver portscan -host 127.0.0.1 -ports 8000-8005
...
```sh
./sotaserver toolbox -check -os pkg
```
...
## Optional: run sotaserver from anywhere

Copy the binary onto your `PATH` so you can type `sotaserver` from any folder:

```sh
mkdir -p $PREFIX/bin
cp sotaserver $PREFIX/bin/
sotaserver help
```
...
./sotaserver subenum -domain example.com -wordlist ~/storage/downloads/subs.txt
```
...
- **Browser can't reach `127.0.0.1:8787`** — make sure `./sotaserver serve` is
...
- **Want to reach it from another device** — run
  `./sotaserver serve -addr 0.0.0.0:8787` and browse to your phone's Wi-Fi IP
