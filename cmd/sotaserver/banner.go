...
func runBanner(ctx context.Context, args []string) error {
	...
	fs.Usage = func() {
		fmt.Fprintf(fs.Output(), "Usage: sotaserver banner -host HOST -port PORT [flags]\n\n")
		fmt.Fprintf(fs.Output(), "Grab a service banner. Examples:\n")
		fmt.Fprintf(fs.Output(), "  sotaserver banner -host example.com -port 22\n")
		fmt.Fprintf(fs.Output(), "  sotaserver banner -host example.com -port 80 -probe \"HEAD / HTTP/1.0\\r\\n\\r\\n\"\n\nFlags:\n")
		fs.PrintDefaults()
	}
