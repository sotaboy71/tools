# Sota's Pen Testing — Android WebView wrapper

A minimal Android app that wraps the sotaserver **web UI** in a WebView, so you
...
the UI served by the sotaserver binary's `serve` command:

```
WebView  ──▶  http://127.0.0.1:8787/  ──▶  sotaserver serve (running on-device)
```

So the workflow is:

1. Run the sotaserver server on the phone (easiest via Termux — see
   [../TERMUX.md](../TERMUX.md)):
   ```sh
   ./sotaserver serve
   ```
...
- **Server address/port** — edit `SERVER_URL` in
  `app/src/main/java/com/sotaserver/app/MainActivity.java` if you run
  `sotaserver serve` on a different port or host.
...
android/
  ...
  app/
    ...
    src/main/
      AndroidManifest.xml
      java/com/sotaserver/app/MainActivity.java
