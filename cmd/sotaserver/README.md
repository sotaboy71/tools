...
Sota's Pen Testing is a set of small tools for **looking at** computers and websites to
...
**To build and run sotaserver itself, you need exactly two things:**

1. **Go** (version 1.11 or newer) — **because** sotaserver is written in the Go
   ...
   <https://go.dev/dl/>
2. **A terminal** — **because** sotaserver is a command-line program: you start
...
That's the whole list. sotaserver uses **only Go's standard library**, so there
...
sotaserver does **not** need Python at all. Python only ever comes up for one
...
because *many other* security tools are written in it — but sotaserver isn't.)

### Optional extras (only for the practice/learning parts — NOT needed to run sotaserver)
...
| **jq** | **Because** sotaserver's `-json` output is compact; jq reformats it to be readable and lets you pull out specific fields. | `apt/brew/pkg install jq` |
...
go build -o sotaserver ./cmd/sotaserver
# or
go install golang.org/x/tools/cmd/sotaserver@latest
```
...
```
sotaserver serve -addr 0.0.0.0:8787
...
Run `sotaserver <command> -h` to see the options for any command. Add `-json` to
...
sotaserver portscan -host 10.0.0.5 -ports 1-1024

# Scan a specific set of ports
sotaserver portscan -host 10.0.0.5 -ports 22,80,443,8080

# Identify a service by its banner
sotaserver banner -host 10.0.0.5 -port 22

# Send an HTTP probe and read the response
sotaserver banner -host 10.0.0.5 -port 80 -probe "HEAD / HTTP/1.0\r\n\r\n"

# Audit a site's security headers
sotaserver httpheaders -url https://example.com
...
sotaserver subenum -domain example.com -wordlist subdomains.txt

# Content discovery: probe common paths and show non-404 responses
sotaserver httpprobe -url https://example.com
sotaserver httpprobe -url https://example.com -wordlist paths.txt -all

# Any command can emit JSON for scripting
sotaserver dns -domain example.com -json | jq '.a'

# Learn to detect and defend the 12 most common attack types
sotaserver attacks                 # list them
sotaserver attacks -name phishing  # detail: what it is, detect, defend
```

## The bigger toolkit (`toolbox`)

sotaserver is a small, safe **learner's** toolkit. The professionals use a wider
set of programs — and `sotaserver toolbox` is a built-in directory of them:

```sh
sotaserver toolbox                 # list everything, grouped by category
sotaserver toolbox -category web   # just one category
sotaserver toolbox -check          # which of them are installed on THIS machine
sotaserver toolbox -json           # as data
```

`sotaserver toolbox -check` looks on your system's `PATH` and reports which of
...
```sh
sotaserver toolbox -check              # hints for the OS you're on now
sotaserver toolbox -check -os brew     # show macOS (Homebrew) commands
```

That's how sotaserver "knows about" the bigger toolkit — it can't contain Kali's
...
**Important:** sotaserver does **not** ship these — they're separate projects.
The directory tells you what each is, links to its official source, and (where
relevant) which sotaserver command is a lightweight version of it. The easiest
...
`sotaserver attacks` is a built-in, read-only reference covering the 12 most
