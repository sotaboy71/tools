...
things with. You don't need all of them — see "What pentoolkit needs" at the
end.
...
- **Shell** (`sh`, `bash`, `zsh`) — the program *inside* the terminal that
  ...
  commands.
- **`./pentoolkit`** — the leading `./` means "run the program in this folder."
  On Windows it's just `pentoolkit.exe`.
- **Go** (`go build`) — the programming language pentoolkit is written in.
  `go build -o pentoolkit ./cmd/pentoolkit` turns the source code into the
  runnable `pentoolkit` program. **This is the only thing pentoolkit itself
...
- **Python** & **`pip`** — Python is a different programming language; `pip` is
  Python's package installer (`pip install <name>`). pentoolkit does **not** use
...
- **jq** — a small tool that pretty-prints and filters JSON; pair it with
  `-json`, e.g. `pentoolkit dns -domain x -json | jq`.
...
- **PATH** — the list of folders your shell searches for programs. Copy
  `pentoolkit` into one (e.g. `/usr/local/bin` or Termux's `$PREFIX/bin`) to run
  it from anywhere by name instead of `./pentoolkit`.

**What pentoolkit needs:** just **Go** to build it (or a prebuilt binary someone
