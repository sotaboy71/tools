...
func runAttacks(ctx context.Context, args []string) error {
	...
	fs.Usage = func() {
		fmt.Fprintf(fs.Output(), "Usage: pentoolkit attacks [-name NAME] [-json]\n\n")
	...
	if *name == "" {
		...
		if len(selected) == 0 {
			return fmt.Errorf("no attack matches %q (try: pentoolkit attacks)", *name)
		}
	}

	return emit(*jsonOut, selected, func() {
		if *name == "" {
			fmt.Println("pentoolkit — threat reference (defender view)")
			fmt.Println("The 12 most common attack types. Run with -name <id> for detail.")
			fmt.Println()
			for _, a := range attackCatalog {
				fmt.Printf("  %-13s %s\n", a.ID, a.Name)
			}
			fmt.Println("\nExample: pentoolkit attacks -name phishing")
			return
		}
